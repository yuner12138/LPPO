...
class PDTSPModel_unvisted_MLP_padavg(nn.Module):
    ...
    def forward(self, state, greedy_construction=False, EAS_incumbent_action=None):
        ...
        if self.force_first_move and state.current_node is None:
            ...
            end_decoder = time.perf_counter()
            print(f"decoder代码运行时间: {end_decoder - start_decoder:.6f} 秒")
...
class PDTSP_Encoder(nn.Module):
    ...
    def forward(self, data):
        ...
        for layer in self.layers:
            ...
            print(f"encoder代码运行时间: {end_encoder - start_encoder:.6f} 秒")
...
class PDTSP_Decoder(nn.Module):
    ...
    def forward(self, all_embedding,encode_emb, encoded_last_node,state,k, ninf_mask):
        ...
        start_unvist = time.perf_counter()
        ...
        end_unvist = time.perf_counter()
        print(f"unvist代码运行时间: {end_unvist - start_unvist:.6f} 秒")
        ...
        print(f"MLP代码运行时间: {end_MLP - start_MLP:.6f} 秒")
        ...
        #######################################################
        # print("mh_atten",mh_atten_out.size())
        # print("single",self.single_head_key.size())
        start_matmul = time.perf_counter()
        score = torch.matmul(mh_atten_out, self.single_head_key)
        end_matmul = time.perf_counter()
        print(f"matmul代码运行时间: {end_matmul - start_matmul:.6f} 秒")
...
def search_unvisited(current, distance, k, masked=None):
    ...
    # 获取当前位置索引
    current_indices = current.reshape(-1)
...
def multi_head_attention(q, k, v, rank2_ninf_mask=None, rank3_ninf_mask=None):
    ...
    start_inMHA_mat = time.perf_counter()

    score = torch.matmul(q, k.transpose(2, 3))

    end_inMHA_mat = time.perf_counter()
    print(f"_inMHA_mat代码运行时间: {end_inMHA_mat - start_inMHA_mat:.6f} 秒")
    # shape: (batch, head_num, n, problem)
    #print("score",score.size())
    start_inMHA_mask = time.perf_counter()
    score_scaled = score / torch.sqrt(torch.tensor(key_dim, dtype=torch.float))
    end_inMHA_mask = time.perf_counter()
    print(f"_inMHA_mask代码运行时间: {end_inMHA_mask - start_inMHA_mask:.6f} 秒")
    ...
    start_inMHA_soft = time.perf_counter()
    weights = nn.Softmax(dim=3)(score_scaled)
    # shape: (batch, head_num, n, problem)
    end_inMHA_soft = time.perf_counter()
    print(f"_inMHA_soft代码运行时间: {end_inMHA_soft - start_inMHA_soft:.6f} 秒")

    start_inMHA_kv = time.perf_counter()
    ...
    # shape: (batch, n, head_num*key_dim)
    end_inMHA_kv = time.perf_counter()
    print(f"_inMHA_kv代码运行时间: {end_inMHA_kv - start_inMHA_kv:.6f} 秒")
