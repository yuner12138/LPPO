...
class PDTSPModel_cluster(nn.Module):
    ...
    def pre_forward_C(self, reset_state, binary_string_pool):
        self.encoded_nodes = self.encoder(reset_state.problems)
        self.encoded_graph = self.encoded_nodes.mean(dim=1, keepdim=True)
        # shape: (batch, problem, EMBEDDING_DIM)
        self.decoder.set_kv_C(self.encoded_nodes)
        self.collect.set_collect_kv(binary_string_pool, self.encoded_graph.size(0), self.encoded_graph)
    ...
    def forward(self, state, greedy_construction=False, EAS_incumbent_action=None):
        ...
        if self.force_first_move and state.current_node is None:
            ...
            probs = self.decoder(self.encoded_graph, encoded_last_node,node_cluster_embed=clust_labels, ninf_mask=state.ninf_mask)
...
def _get_encoding(encoded_nodes, node_index_to_pick):
    ...
    # print("encoded_nodes",encoded_nodes.size())
    # print("node_index_to_pick",node_index_to_pick.size())
...
class PDTSP_Collect(nn.Module):
    def __init__(self, **model_params):
        ...
        self.C_Wq = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.C_Wk = nn.Linear(self.model_params['z_dim'], head_num * qkv_dim, bias=False)
        self.C_Wv = nn.Linear(self.model_params['z_dim'], head_num * qkv_dim, bias=False)
        self.C_Single = nn.Linear(self.model_params['z_dim'], head_num * qkv_dim, bias=False)
        self.C_multi_head_combine = nn.Linear(head_num * qkv_dim, embedding_dim)

        self.C_k = None  # saved key, for multi-head attention
        self.C_v = None  # saved value, for multi-head_attention
        self.C_single_head_key = None  # saved, for single-head attention
        self.encoded_graph = None
    ...
    def set_collect_kv(self, binary_string_pool, batch_size, encoded_graph):
        # encoded_nodes.shape: (batch, problem+1, embedding)
        self.binary_string_pool = binary_string_pool

        kv_binary_string_pool = binary_string_pool.unsqueeze(0).repeat(batch_size, 1, 1)
        # print('kv_pool={}'.format(kv_binary_string_pool.size()))
        head_num = self.model_params['head_num']

        self.C_k = reshape_by_heads(self.C_Wk(kv_binary_string_pool), head_num=head_num)
        # print('C_k={}'.format(self.C_k.size()))
        self.C_v = reshape_by_heads(self.C_Wv(kv_binary_string_pool), head_num=head_num)
        # shape: (batch, head_num, x, qkv_dim)
        self.C_single_head_key = self.C_Single(kv_binary_string_pool).transpose(1, 2)
        # shape: (batch, embedding, x)
    ...
    def forward(self, z_sample):
        ...
        # shape: (batch, 2**bit, bit)
        q = reshape_by_heads(self.C_Wq(self.encoded_graph), head_num=head_num)
        # shape: (batch, head_num, 1, qkv_dim)

        out_concat = self.attention_fn(q, self.C_k, self.C_v)
        # shape: (batch, 1, head_num*qkv_dim)

        mh_atten_out = self.C_multi_head_combine(out_concat)

        # shape: (batch, 1, embedding)

        #  Single-Head Attention, for probability calculation
        #######################################################
        score = torch.matmul(mh_atten_out, self.C_single_head_key)
        # shape: (batch, 1, x)
        ...
        probs = F.softmax(score_clipped, dim=2)
        # shape: (batch, 1, x)
        indices = probs.squeeze(1).multinomial(z_sample, replacement=False)
        z = self.binary_string_pool[indices]
        return z
...
#         self.use_EAS_layers = False
#
#         self.C_Wq = nn.Linear(self.model_params['z_dim'], head_num * qkv_dim, bias=False)
#         self.C_Wk = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
#         self.C_Wv = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
#
#         self.C_multi_head_combine = nn.Linear(head_num * qkv_dim, embedding_dim)
#
#         self.C_k = None  # saved key, for multi-head attention
...
#             self.attention_fn = multi_head_attention
#
#     def set_collect_kv(self, encoded_nodes):
#         # encoded_nodes.shape: (batch, problem+1, embedding)
#         head_num = self.model_params['head_num']
#
#         self.C_k = reshape_by_heads(self.C_Wk(encoded_nodes), head_num=head_num)
#         self.C_v = reshape_by_heads(self.C_Wv(encoded_nodes), head_num=head_num)
#         # shape: (batch, head_num, problem+1, qkv_dim)
#         self.C_single_head_key = encoded_nodes.transpose(1, 2)
#         # shape: (batch, embedding, problem+1)
#
#
#     def forward(self, encoded_graph, binary_string_pool):
#         # encoded_last_node.shape: (batch, pomo, embedding)
#         # ninf_mask.shape: (batch, pomo, problem)
#
#         head_num = self.model_params['head_num']
...
#         # q_last = reshape_by_heads(self.Wq_last(encoded_last_node), head_num=head_num)
#         # shape: (batch, head_num, pomo, qkv_dim)
#         query = binary_string_pool.unsqueeze(0).repeat(batch_size, 1, 1)
#         # shape: (batch, 2**bit, bit)
#         q = reshape_by_heads(self.C_Wq(query), head_num=head_num)
#         # shape: (batch, head_num, 2**bit, qkv_dim)
#
#         out_concat = self.attention_fn(q, self.k, self.v)
#         # shape: (batch, 2**bit, head_num*qkv_dim)
#
#         mh_atten_out = self.C_multi_head_combine(out_concat)
#
#         # shape: (batch, 2**bit, embedding)
#
#
#         #  Single-Head Attention, for probability calculation
#         #######################################################
...
class PDTSP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Wq_clu = nn.Linear(self.embedding_dim + self.num_clusters*self.embedding_dim, head_num * qkv_dim, bias=False)
        ...
        self.hyper_Wq_last = DiagonalMLP(self.embedding_dim + self.embedding_dim * self.num_clusters,
                                         self.embedding_dim, self.embedding_dim)
        self.cluster_embed = EM_LayerCont(self.embedding_dim, self.embedding_dim, num_clusters=self.num_clusters,
    ...
    def _update_unvisited(self, embedding, cluster_weights):
        ...
        # print("cluster_weights",cluster_weights.size())
        # print("cluster_reps",self.cluster_reps.size())
    ...
    def forward(self, all_embedding, encoded_last_node,node_cluster_embed, ninf_mask):
        ...
        # q_last = reshape_by_heads(self.Wq_last(encoded_last_node), head_num=head_num)
        # shape: (batch, head_num, pomo, qkv_dim)
        #unvist_node_emb = search_unvist(state.current_node,state.distance)
        ...
        #emphasize_cat = torch.cat((encoded_last_node, unvist_node_emb), dim=2)


        q = reshape_by_heads(self.Wq_clu(input_cat), head_num=head_num)
        # shape: (batch, head_num, pomo, qkv_dim)
        ...
        # shape: (batch, rollout, embedding)
        weights = self.hyper_Wq_last(input_cat)
        # print(mh_atten_out.size())
...
def search_unvist(current_node,distance,k=10):

    # 初始化输出张量 c
    c = torch.empty(current_node.size(0), k, dtype=torch.long)

    # 对每个批次进行处理
    for i in range(current_node.size(0)):
        # 获取当前批次的位置
        current_pos = current_node[i]

        # 获取当前批次的距离矩阵
        distance_matrix = distance[i]

        # 由于距离矩阵是对称的，我们只需要考虑上三角（或下三角）来避免重复计算
        # 但为了简化，这里我们直接对整个矩阵操作，并稍后排除自身

        # 计算当前位置到其他所有位置的距离（排除自身）
        distances = distance_matrix[current_pos, :].clone()  # 复制一行以避免修改原始数据
        distances[current_pos] = float('inf')  # 将当前位置的距离设置为无穷大，以便在排序时被排除

        # 对距离进行排序并获取索引（注意：这里我们获取的是值的索引，而不是直接的距离）
        _, indices = torch.sort(distances, descending=False)

        # 由于我们排除了当前位置，所以索引是从 0 开始的，但我们只需要前 k 个（除了被设置为无穷大的那个）
        # 注意：如果 k 大于或等于 problem_size - 1，则可能需要调整以避免索引越界
        indices = indices[:k]
