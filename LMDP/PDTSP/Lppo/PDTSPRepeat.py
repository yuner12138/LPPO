...
class PDTSPRepeat:
    ...
    def _test_one_batch(self, batch_size):
        ...
        with torch.no_grad():
            ...
            #test
            z_idx = torch.tensor([self.tester_params['vector']])
            expanded_z_idx = z_idx.repeat(z_sample_size)
            z = self.binary_string_pool[expanded_z_idx].reshape(batch_size * aug_factor, starting_points, z_sample_size, z_dim)
        ...
        print('reward={}'.format(reward.topk(10, dim=1)))
        a = -reward
        b = (a < 9.6601).sum().item()
        print('优于greedy的数量{p1},比列{p2}'.format(p1=b,p2=b/100000))
        # Return
        ###############################################
