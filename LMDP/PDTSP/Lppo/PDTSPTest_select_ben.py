...
class PDTSPTest_select_ben:
    ...
    def run(self):
        ...
        if self.tester_params['test_data_load']['enablepkl']:
            self.env.use_pkl_saved_problems(self.tester_params['test_data_load']['filenamepkl'], test_num_episode)
        # elif self.tester_params['test_data_load']['enablecvs']:
        ...
        while iter < 5:
            ...
            batch_size = self.tester_params['test_batch_size1']
            ...
            max_aug_obj = min(max_aug_obj,aug_score)
            max_avg_obj = min(max_avg_obj, score)
    ...
    def _test_one_batch(self, batch_size):
        ...
        with torch.no_grad():
            ...
            if self.tester_params['greedy_select_block']:
                self.env.get_dataset_problem(self.tester_params['test_data_load']['filenamecvs'], batch_size,self.k*starting_points, aug_factor)
                reset_state, group_state, _, _ = self.env.reset()
            if self.tester_params['usekmeans']:
                if self.tester_params['greedy_select_block']:
                    # # 将NumPy数组转换为PyTorch张量 (原代码)
                    z = self.closest_vectors
                    z = z.unsqueeze(0).repeat(batch_size * aug_factor*starting_points, 1,1).reshape(batch_size * aug_factor, starting_points, self.k, z_dim)
                    z = z.to(device)
                    #print("z size",z.size())
            ...
            if self.tester_params['augmentation_enable']:
                if self.tester_params['greedy_select_block']:
                    z = z.transpose(1, 2).reshape(batch_size * aug_factor, self.k*starting_points, z_dim)
                else:
        ...
        print("reward1.size", reward.size())
        # print("reward1",reward)
        print("reward max",torch.max(reward))
        if not self.tester_params['greedy_select_block']:
        ...
        with torch.no_grad():
            self.env.get_dataset_problem(self.tester_params['test_data_load']['filenamecvs'], batch_size, rollout_size,
                                         aug_factor)
            reset_state, group_state, _, _ = self.env.reset()
            ...
            # prob = torch.ones(size=(batch_size, self.env.Bit_size))
            first_selected = torch.zeros(size=(batch_size * aug_factor, self.env.rollout_size))
            # print("first_selected",first_selected.size())
            state, reward, done = self.env.step(first_selected)

            # selected = (torch.arange(50)+1).repeat(rollout_size // 50)[None, :].expand(batch_size* aug_factor,rollout_size)
            # # print("selected",selected.size())
            # # print(selected)
            # prob = torch.ones(size=(batch_size* aug_factor, rollout_size))
            ...
            while not done:
                ...
                prob_list = torch.cat((prob_list, prob_clone[:, :, None]), dim=2)
        # print("reward",reward)
        print("reward2.size", reward.size())
        ...
        # print("aug_greedy_reward",aug_greedy_reward)
