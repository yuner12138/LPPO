...
class PDTSPModel_unvisted(nn.Module):
    ...
    def forward(self, state, greedy_construction=False, EAS_incumbent_action=None):
        ...
        if self.force_first_move and state.current_node is None:
            ...
            probs = self.decoder(self.encoded_graph,self.encoded_nodes, encoded_last_node,state,self.k_near, ninf_mask=state.ninf_mask)
...
class PDTSP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Wq_clu = nn.Linear(self.embedding_dim + self.max_unvisited*self.embedding_dim, head_num * qkv_dim, bias=False)
        ...
        self.hyper_Wq_last = DiagonalMLP(self.embedding_dim + self.embedding_dim * self.max_unvisited,
    ...
    def forward(self, all_embedding,encode_emb, encoded_last_node,state,k, ninf_mask):
        ...
        unvist_node_index,_ = search_unvisited(state.current_node,state.distance,k,ninf_mask)
        # print("unvist_node_index",unvist_node_index)
        # print(unvist_node_index.size())
        unvist_node_index = unvist_node_index.reshape(b,-1)
        unvist_node_emb = _get_encoding(encode_emb, unvist_node_index)
    ...
    for i in range(batch_size):
        for j in range(rollout_size):
            ...
            if len(k_nearest_indices)!=k:
                ...
                padding_tensor = torch.full((k - k_nearest_indices.size(0),), fill_value=current_index, dtype=torch.int32)
...
def search_unvisited(current, distance, k, masked=None):
    ...
    return new_tensor, padding_size
...
def search_unvist_no(current_node,distance,k=10):

    # 初始化输出张量 c
    print("distance",distance.size())
    print(current_node.size())
    current_node = current_node.view(-1)
